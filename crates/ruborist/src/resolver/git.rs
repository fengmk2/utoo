...
//! It provides:
//! - Low-level clone + cache logic ([`clone_repo`])
...
/// `downloader.rs`.
static GIT_CLONE_CACHE: LazyLock<
    Mutex<HashMap<String, Arc<tokio::sync::OnceCell<GitCloneResult>>>>,
            ...
            gix::object::tree::EntryKind::Blob | gix::object::tree::EntryKind::BlobExecutable => {
                ...
                if entry.mode().kind() == gix::object::tree::EntryKind::BlobExecutable {
                    ...
                    if let Err(e) = std::fs::set_permissions(&entry_path, perms) {
                        tracing::warn!("Failed to set executable permission on {}: {e}", entry_path.display());
                    }
                }
            }
...
fn clone_repo_blocking(
    ...
    // Validate package name to prevent path traversal (e.g. `../..`)
    ...
    if package_dir.exists() {
        std::fs::remove_dir_all(&package_dir)
            .context("Failed to clean existing cache directory before extraction")?;
    }
...
/// 2. Clones the repository via [`clone_repo`].
...
pub(crate) async fn resolve_non_registry_dep(
    ...
    let result = clone_repo(cache_dir, &url, commit_ish.as_deref()).await?;
...
async fn build_manifest_from_git_cache(
    name: &str,
    version: &str,
    cache_path: &Path,
    resolved_url: &str,
    ...
    let pkg_json_content = tokio::fs::read_to_string(&pkg_json_path)
        .await
